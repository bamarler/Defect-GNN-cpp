...
class GNN:
    def __init__(
        ...
        if modelname == "CGNN":
            ...
            assert num_hidden_channels % num_heads == 0, (
                f"{modelname} network must have hidden channels "
    ...
    def splitTrainValTest(self, dataset, batchsz, train_ratio=0.8, val_ratio=0.1):
        ...
        val_loader = DataLoader(
            dataset[train_size : train_size + val_size], batch_size=batchsz
        )
    ...
    def run(self):
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
            self.optimizer, "min", min_lr=1e-5
        )
        ...
        for epoch in range(self.max_epoch):
            ...
            if val_loss < self.min_val_loss:
                ...
                self.logger.info(
                    f"Early stopping step {self.early_schedule_step}, the current validation loss {val_loss}"
...
class GNN_optuna:
    ...
    def objective(self, trial):
        modelname = trial.suggest_categorical(
            "modelname", ["CGNN", "GAT", "Transformer"]
        )
        num_hidden_layers = trial.suggest_int("num_hidden_layers", 1, 8)
        num_hidden_channels = trial.suggest_categorical(
            "num_hidden_channels", [8, 16, 32, 64, 128, 256]
        )
