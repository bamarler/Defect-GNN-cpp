...
def enlarge_cell(data_dir, id, r_cutoff):
    ...
    for typ, vec in enlarge_dict.items():
        for v in vec:
            ...
            atom_index += 1
    with open(
        data_dir + "/atoms/" + id + "_" + str(r_cutoff) + "_enlarge.npz", "wb"
    ) as out_file:
...
def get_betti_num(data_dir, id, r_cutoff):
    if os.path.exists(data_dir + "/betti_num/" + id + "_" + str(r_cutoff)):
        return "exists"
    with open(
        data_dir + "/atoms/" + id + "_" + str(r_cutoff) + "_enlarge.npz", "rb"
    ) as structfile:
        data = np.load(structfile)
    ...
    for cav in center_atom_vec:
        ...
        for ele in typ_dict.keys():
            ...
            for i in range(len(cart_enlarge_vec)):
                ...
                if (
                    vec["typ"].decode() == ele
                    and np.linalg.norm(vec["pos"][:] - cav["pos"][:]) <= r_cutoff
                ):
                    pair_index.append(i)
            if len(pair_index) == 0:
                continue
...
def getElementalBettiProperties(lines, typ_dict):
    ...
    Bar0Death = np.asarray(Bar0Death)
    Bar1Birth = np.asarray(Bar1Birth)
    Bar1Death = np.asarray(Bar1Death)
    Bar2Birth = np.asarray(Bar2Birth)
    Bar2Death = np.asarray(Bar2Death)
    WBar0Death = np.asarray(WBar0Death)
    WBar1Birth = np.asarray(WBar1Birth)
    WBar1Death = np.asarray(WBar1Death)
    WBar2Birth = np.asarray(WBar2Birth)
    WBar2Death = np.asarray(WBar2Death)
    Feature_2 = []
    # Betti0
    if len(Bar0Death) > 0:
...
if __name__ == "__main__":
    ...
    with multiprocessing.Pool(processes=num_processes) as pool:
        pool.starmap(
            calculateBettiNumber,
            [
                (chunk, data_dir, r_cutoff, chunk_id)
                for chunk_id, chunk in enumerate(id_chunks)
            ],
        )
