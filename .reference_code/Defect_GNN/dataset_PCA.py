...
def getDFTEnergy(DFT_dir):
    assert "data.csv" in os.listdir(DFT_dir), (
        f"data.csv file does not exist in {DFT_dir}."
    )
...
def getAtomicFeatures(root, atomic_feature_type):
    if "number" in atomic_feature_type:
        ...
        assert "atomic_embedding_CGCNN.json" in os.listdir(root), (
            f"cannot read atomic_embedding_CGCNN.json file from {root}"
        )
...
def getDistanceToVacancy(pris_struct, defect_struct):
    ...
    vac_pos = finder.get_defect_fpos(
        defect_structure=defect_struct, base_structure=pris_struct
    )
...
class Dataset(InMemoryDataset):
    ...
    def process(self):
        ...
        atom_dict = getAtomicFeatures(
            root=DFT_dir, atomic_feature_type=self.atomic_feature_type
        )
        ...
        for _, row in tqdm(df.iterrows(), total=len(df)):
            ...
            if self.r_cutoff_ripser == 0:
                ...
                topo_features.append(
                    getElementalBettiFeatures(
                        data_dir=f"{DFT_dir}/Betti_number/{self.r_cutoff_ripser}",
                        id=f"{pris_idx}_{vac_idx}",
                        r_cutoff=self.r_cutoff_ripser,
                    ).numpy()
                )
        ...
        for _, row in tqdm(df.iterrows(), total=len(df)):
            ...
            pris_idx, vac_idx = row["pris_idx"], row["vac_idx"]
            pris_structure = Structure.from_file(
                f"{DFT_dir}/pristine_structures/{pris_idx}.vasp"
            )
            def_structure = Structure.from_file(
                f"{DFT_dir}/defective_structures/{pris_idx}_{vac_idx}.vasp"
            )

            dist_to_vac = getDistanceToVacancy(
                pris_struct=pris_structure, defect_struct=def_structure
            )

            if self.r_cutoff_ripser == 0:
                topo_features = None
            else:
                topo_features = getElementalBettiFeatures(
                    data_dir=f"{DFT_dir}/Betti_number/{self.r_cutoff_ripser}",
                    id=f"{pris_idx}_{vac_idx}",
                    r_cutoff=self.r_cutoff_ripser,
                )
            ...
            data = structureToGraph(
                structure=def_structure,
                E=formation_energy,
                topo_features=topo_features,
                atom_dict=atom_dict,
                dist_to_vac=dist_to_vac,
                dr=self.dr,
            )
...
if __name__ == "__main__":
    ...
    for PCA_components in [1, 2, 4, 6, 8, 10]:
        ...
        dataset = Dataset(
            root=root,
            r_cutoff_ripser=r_cutoff_ripser,
            atomic_feature_type=atomic_feature_type,
